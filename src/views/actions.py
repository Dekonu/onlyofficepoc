...
def downloadfullhistory(request):
        ...
        folder_path = docManager.getHistoryDirPath(fileName, request)
        compressed_history = shutil.make_archive(
            base_name=f"{fileName}-hist",
            format="gztar",
            root_dir=folder_path,
        )
        return docManager.download(compressed_history)

# download a history file
def downloadhistoryfile(request):
    try:
        fileName = fileUtils.getFileName(request.GET['fileName'])  # get the file name
        userAddress = request.GET.get('userAddress') if request.GET.get('userAddress') else request
